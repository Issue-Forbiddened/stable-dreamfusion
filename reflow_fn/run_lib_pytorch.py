...
from reflow_fn.utils_ import save_checkpoint, restore_checkpoint
...
def evaluate(config,
  ...
  Args:
    ...
    eval_folder: The subfolder for storing evaluation results. Default to
      "eval".
  """
  if getattr(config.eval, "image_only", False):
    evaluate_image_only(config, workdir, eval_folder)
    return
...
def evaluate_image_only(config,
             workdir,
             eval_folder="eval"):
  """Evaluate trained models.
  ...
  for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
    ...
    if config.eval.enable_figures_only:
      ...
      for r in range(num_sampling_rounds):
        ...
        this_sample_dir=this_sample_dir+f"_sampler_{config.sampling.use_ode_sampler}_step_{config.sampling.sample_N}"
        os.makedirs(this_sample_dir, exist_ok=True)
        
        # Directory to save samples. Different for each host to avoid writing conflicts
        samples, n = sampling_fn(score_model)
        torchvision.utils.save_image(samples.clamp_(0.0, 1.0), os.path.join(this_sample_dir, '%d.png'%r), nrow=8, normalize=False)
